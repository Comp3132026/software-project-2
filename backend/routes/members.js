...
router.post('/group/:groupId', auth, async (req, res) => {
  try {
    ...
    if (!['member', 'viewer'].includes(role)) {
      return res.status(400).json({ message: 'Invalid role. Must be member or viewer.' });
    }
...
router.put('/group/:groupId/:userId/role', auth, async (req, res) => {
  try {
    ...
    const caller = group.members.find((m) => m.user?.toString() === req.userId.toString());
    if (!caller) {
    ...
    if (callerRole === 'moderator' && target.user.toString() === group.owner.toString()) {
      return res.status(403).json({ message: 'Moderators cannot modify the owner.' });
    }

    // Moderators cannot assign owner role
    if (callerRole === 'moderator' && role === 'owner') {
      return res.status(403).json({ message: 'Moderators cannot assign owner role.' });
    }
...
router.put('/group/:groupId/:userId/suspend', auth, async (req, res) => {
  try {
    ...
    if (group.owner.toString() !== req.userId.toString()) {
      return res.status(403).json({ message: 'Only the group owner can suspend members.' });
    }
...
router.delete('/group/:groupId/:userId', auth, async (req, res) => {
  try {
    ...
    await Task.updateMany({ group: groupId }, { $pull: { assignedTo: userId } });
