...
router.post("/", auth, async (req, res) => {
  try {
    ...
    if (member && member.isSuspended) {
      return res
        .status(403)
        .json({ message: "You are suspended and cannot send messages." });
    }
...
router.put("/:id", auth, async (req, res) => {
  try {
    ...
    if (!content || !content.trim()) {
      return res
        .status(400)
        .json({ message: "Updated message content is required." });
    }
    ...
    if (message.isDeleted) {
      return res
        .status(400)
        .json({ message: "Deleted messages cannot be edited." });
    }
...
router.delete("/:id", auth, async (req, res) => {
  try {
    ...
    const group = await Group.findById(message.group._id).populate(
      "members.user",
    );
...
router.post("/:id/warn", auth, async (req, res) => {
  try {
    ...
    if (message.type === "warning") {
      return res
        .status(400)
        .json({ message: "This message is already marked as a warning." });
    }
