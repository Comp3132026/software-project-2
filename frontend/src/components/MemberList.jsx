...
export default function MemberList({ groupId, members = [], isOwner, canManageMembers, onUpdate }) {
  ...
  const canManageMember = (member) => {
    ...
    if (currentUser?.role === 'moderator' && member.role === 'moderator') return false;
    return true;
  };
  ...
  return (
    <div>
      ...
      <div className="space-y-2">
        {members.length === 0 ? (
          <div className="text-center py-6 text-surface-300 text-sm bg-surface-50 rounded-xl">
          ...
          members.map((m) => (
            <div
              ...
              onClick={() => {
                ...
                navigate(`/group/${groupId}/member/profile/${m.userId}`);
              }}
            >
              <div className="flex items-center gap-3">
                ...
                >
                  <span
                    className={`text-sm font-bold ${
                      m.isSuspended ? 'text-red-500' : 'text-white'
                    }`}
                  >
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    {getRoleIcon(m.role)}
                    <span className="text-xs text-surface-300 capitalize">{m.role}</span>
                  </div>
